...
async function advice(body, env) {
  ...
  const result = await callClaude(env, {
    ...
    system:
      ...
      "【キャラと呼び名】ユーザープロンプトに【キャラ設定】や【呼び名】の指定があれば必ずそれに従う。" +
      "指定された口調は headline から advice の最後まで崩さず、丁寧語には戻らない。" +
      "呼び名が指定されている場合は「あなた」「ユーザーさん」などを使わず、必ずその呼び名で呼びかける。\n\n" +
