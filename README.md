...
キャラクターは画像ファイルではなく**インラインSVGで描画**している。表情・体型・レベル・タイプごとの
パーツをSVG内に持たせ、CSSクラスで出し分けるため、状態が増えても画像を増やさずに済む
(4タイプ × 4表情 × 3体型 × 5段階 = 240通りを画像0枚で表現している)。

好みで選べる4タイプ。見た目だけでなく、アプリ内の声かけとAIに渡す口調の指示も変わる:

<img src="docs/types.png" width="640" alt="王道ギャル・ナチュラル系・モード系・姉御ギャルの4タイプ">
...
<img src="docs/settings.png" width="260" alt="呼び名とギャルタイプを選ぶ設定画面">
...
- **なかよし度(レベル)** — 記録するほどギャルと仲良くなり、Lv3でピアス、Lv6でインナーカラー、Lv10でティアラ👑、Lv15で伝説のギャルに
- **ギャルのタイプを選べる** — 王道ギャル / ナチュラル系 / モード系 / 姉御ギャルの4種。髪・肌・服・メイクだけでなく、褒め方のテンションも変わる
- **呼び名を設定できる** — 「ひろくん」「そうちゃん」のように登録すると、アプリ内の声かけもAIの返答も必ずその呼び名で呼びかけてくれる
...
- **フロント**: 依存ライブラリなしの HTML / CSS / JavaScript(ビルド不要)。キャラクターはインラインSVG + CSSカスタムプロパティのみで描画
...
.
├── index.html        # 画面
├── style.css         # ギャル系テーマ + キャラクター描画(タイプ別の配色もここ)
├── app.js            # 記録・カレンダー・なかよし度・表情/体型/タイプ・フィードバック
├── api.js            # 中継サーバー呼び出し
